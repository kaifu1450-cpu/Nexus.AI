...
exports.handler = async function (event) {
  ...
  try {
    ...
    // 5. Update usage (tracking only, no limits)
    const today = new Date().toISOString().split('T')[0];
    let { data: usage } = await supabase.from('usage_tracking').select('*').eq('user_id', user.id).single();
    await supabase.from('usage_tracking').upsert({
      user_id:             user.id,
      daily_message_count: (usage?.last_reset_date === today ? (usage?.daily_message_count || 0) : 0) + 1,
    ...
    return {
      ...
      body: JSON.stringify({
        reply,
        usage: { used: 1, limit: 999999, remaining: 999999, plan: 'free' },
      }),
    };

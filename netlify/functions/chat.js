...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (!auth.startsWith('Bearer ')) return { statusCode: 401, headers, body: JSON.stringify({ error: 'Please login to continue' }) };

    const token = auth.split(' ')[1];
    const supabase = createClient(process.env.VITE_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);

    const { data: { user }, error: authErr } = await supabase.auth.getUser(token);
    if (authErr || !user) return { statusCode: 401, headers, body: JSON.stringify({ error: 'Session expired. Please login again.' }) };
    ...
    if (plan !== 'premium' && count >= limit) {
      return { statusCode: 429, headers, body: JSON.stringify({ error: 'limit_reached', message: `Daily limit of ${limit} messages reached. Please upgrade!` }) };
    }
    ...
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: 'llama-3.3-70b-versatile', messages: groqMessages, max_tokens: 1500, temperature: 0.7 }),
    });

    const data = await res.json();
    if (data.error) return { statusCode: 400, headers, body: JSON.stringify({ error: data.error.message || 'AI error' }) };
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ reply, usage: { used: count + 1, limit, remaining: Math.max(0, limit - count - 1), plan } }) };
